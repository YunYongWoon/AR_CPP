...
/*
링크드리스트 : 자료구조의 한 종류이다. 자료구조란 데이터를 관리하는
방법을 말한다.
- 링크드리스트는 데이터 목록을 연결시켜서 접근할 수 있는 구조를 제공한다.
- 데이터를 저장하기 위한 노드라는 것이 존재하고 각 노드의 특징은 다음 노드를 알 수 있게 된다.
- 리스트는 선형 구조로 되어있다. 그렇기 때문에 배열처럼 특정 요소에 바로
접근이 불가능하다.
- 무조건 앞에서부터 차례대로 타고 들어가야 한다.
- 노드를 새로 추가할때는 노드를 생성하고 마지막 노드에 연결만 해주면
되기 때문에 개수의 제한이 없다.
*/
...
void Delete(PLIST pList) {
	...
	while (pNode != NULL) {
		if (strcmp(pNode->tStudent.strName, strName) == 0) {
			...
			// 이전 노드가 있으면 이전 노드의 다음을 지운 노드 다음노드로 연결
			else {
				delete pNode;
				pPrev->pNext = pNext;
