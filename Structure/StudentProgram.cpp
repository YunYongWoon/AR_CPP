...
int main() {
	...
	while (true) {
		...
		// cin은 오른쪽에 int변수가 오면 정수를 입력해야 한다. 오른쪽에 오는 변수 타입에
		// 맟춰서 값을 입력해야 하는데 실수로 정수가 아닌 문자를 입력할 경우 에러가 발생한다.
		// 이때문에 예외처리로 에러가 발생했는지를 여기에서 체크하여 에러가 발생하면 cin내부의 에러버퍼를 비워주고
		// cin 내부에 입력버퍼가 있는데 입력버퍼는 입력한 값을 저장해놓고 그 갑승ㄹ 변수에 넣어주는 역할을 한다. 이 입력
		// 버퍼에 \n이 남아있으므로 버퍼를 순회하여 \n을 지워준다.

		// 버퍼 : 임시저장곤간이다.
		
		// 에러를 체크한다. cin.fail()을 했을 때 입력 에러가 발생할 경우 true를 반환한다.
		if (cin.fail()) {
			// 에러버퍼를 비워준다.
			cin.clear();

			// 입력버퍼에 \n이 남아있으므로 입력버퍼를 검색하여 \n을 지워준다.
			// 첫번째에는 검색하고자 하는 버퍼 크기를 지정한다. 넉넉하게 1024바이트로 지정해주었다. 
			// 두번째에는 찾고자 하는 문자를 넣어준다. 그래서 입력버퍼에서 \n이 있는 곳까지 찾아서 그 부분을 모두
		...
		if (iMenu == MENU_EXIT)
			break;
		switch (iMenu) {
		// 학생정보를 추가한다.(학번, 이름, 국영수 성적(총점, 평균), 주소, 핸드폰 번호)
		// 국영수 점수는 입력, 학번, 총점, 평균은 연산을 통해 계산해준다.

		//등록 된 학생이 등록할 수 있는 최대치일 경우 더이상 등록이 안되게 막는다.

		case MENU_INSERT:  
			system("cls");
			cout << "=====================학생추가========================" << endl;
			if (iStudentCount == STUDENT_MAX){
				cout << "등록할 공간이 부족합니다." << endl;
				break; 
			}
